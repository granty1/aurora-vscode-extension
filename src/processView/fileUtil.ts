...
export const FileUtil = {
    ...
    createDefaultANesFloder:function(userRoot:string){
        ...
        fs.writeFile(path.join(userRoot, '.cprocess', 'menu', PROCESS_FILE_NAME),"[]",()=>{

        });
    },
    getRomConfigFileList: function(userRoot:string){
        let metaData = fs.readFileSync(path.join(userRoot, '.cprocess', 'menu', PROCESS_FILE_NAME));
	...
	writeMetaInfo:function(userRoot:string, metaInfo:string){
        let pa=path.join(userRoot, '.cprocess', "menu",PROCESS_FILE_NAME)
